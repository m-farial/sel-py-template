...
class WCAGReference:
    ...
    url: str | None = None  # Link to WCAG doc


@dataclass
class AxeNode:
    """A single affected node from axe-core."""
...
class ReportViolation:
    """Violation formatted for HTML report."""
    name: str  # Violation ID (e.g., 'aria-allowed-role')
    summary: str  # Main violation summary line
    help: str  # Help text describing the issue
    help_url: str  # URL to detailed documentation
    nodes: list[AxeNode]  # List of affected nodes
    tags: list[str]  # Tags (e.g., 'wcag21aa', 'cat.forms')
    screenshot: str = ""  # Path or URL to screenshot (optional)
    wcag_refs: list[WCAGReference] = field(default_factory=list)  # WCAG references (optional)
...
class A11yViolationsReport:
    ...
    def add_violation(self, violation: ReportViolation) -> None:
        ...
        Args:
            violation: ReportViolation dataclass instance
        """
        self.violations.append(violation)

    def add_violations_from_axe(self, axe_violations: list[dict]) -> None:
        """
        Add violations directly from axe-core results.

        Args:
            axe_violations: List of violation dicts from axe-core
        """
